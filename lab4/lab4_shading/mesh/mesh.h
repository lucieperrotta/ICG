...
private:
    void BindShader(GLuint program_id) {
        ...
        //vertex_array_id_[2] = texture_1d_id_;
        // Euh ouais j'essaie de la merde mais je comprends pas encore comment ça marche xD
        GLint vertex_texturel_id = glGetAttribLocation(program_id, "vnormal");
        if (program_id == toon_program_id_) {
            glEnableVertexAttribArray(vertex_normal_id);
            glBindBuffer(GL_ARRAY_BUFFER, vertex_texturel_id);
            glVertexAttribPointer(vertex_normal_id, 3 /*vec3*/, GL_FLOAT,
                                  DONT_NORMALIZE, ZERO_STRIDE, ZERO_BUFFER_OFFSET);
        }
